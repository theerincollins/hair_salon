...
describe (Stylist) do
  ...
  describe('#clients') do
    it("displays all clients for one stylist") do
      test_stylist = Stylist.new({:name => "Girl Guy", :id => nil})
      test_stylist.save()
      test_client = Client.new({:name => "Sassy Guy", :id => nil, :type_of_cut => "Buzz Cut", :stylist_id => test_stylist.id()})
      test_client.save()
      test_client2 = Client.new({:name => "Purple People Eater", :id => nil, :type_of_cut => "Full Body Wax", :stylist_id => test_stylist.id()})
      test_client2.save()
